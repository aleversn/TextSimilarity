...
dataiter = DataLoader(myDataset, batch_size=400)
...
class BIMPM(nn.Module):
    def __init__(self, hidden_size=100, num_perspective=20, class_size=2, device="gpu"):
        ...
        self.word_emb = nn.Embedding(49807, 300)
...
bimpm = BIMPM(device=device)
if torch.cuda.is_available():
    bimpm.cuda()
if torch.cuda.device_count() > 1:
    bimpm = nn.DataParallel(bimpm, device_ids=[0, 1, 2, 3])
    bimpm.to(device)
# model_dict = torch.load("./model/esim_x/esim_sim_2.pth").module.state_dict()
# esim.module.load_state_dict(model_dict)

criterion = nn.BCELoss()
optimizer = torch.optim.Adam(bimpm.parameters(), lr=1e-3)
...
for epoch in range(num_epochs):
    ...
    torch.save(bimpm, './model/bimpm/bimpm_sim_{}.pth'.format(epoch + 1 + save_offset))
    WriteSDC('log_bimpm.log', 'epoch: {} train_acc: {} loss: {}\n'.format(epoch + 1 + save_offset, train_acc / train_count, train_loss / train_count))
    
    if epoch == 0 or epoch % 3 != 0:
        continue
    
    eval_bimpm(bimpm, eval_list, myData_eval, epoch, save_offset)

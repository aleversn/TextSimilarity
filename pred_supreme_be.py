...
_, std_dict, _ = Preprocess.load_std('./dataset/std_data')
...
model_dict = torch.load("./model/supreme/l{}/bert_supreme_8.pth".format(LABEL_ID)).module.state_dict()
...
with torch.no_grad():
    ...
    for idx, item in enumerate(eval_list_iter):
        ...
        max_item_index = pred_scores.max(-1)[1].data.item()
        max_item_id = myDataset.cls_id_2_label[max_item_index]

        sentences = torch.tensor([
            T['input_ids'],
            tokenizer(std_dict[max_item_id][2], add_special_tokens=True, max_length=100, padding='max_length', truncation=True)['input_ids']
        ]).unsqueeze(0).cuda()
        out = esim(sentences)
        sim = out[:,1].max(-1)[0]

        # if sim.data.item() < 0.98:
        #     max_item_id = myDataset.cls_id_2_label[pred_scores.sort(descending=True)[1][1].data.item()]

        if int(max_item_id) == int(item[0]):
            c.append(sim.data.item())
            eval_correct_num += 1
            correct_dict.append({'correct_id': item[0], 'pred_ids': [myDataset.cls_id_2_label[i] for i, _ in enumerate(pred_scores.tolist())], 'scores': pred_scores.tolist()})
        else:
            error_dict.append({'correct_id': item[0], 'pred_ids': [myDataset.cls_id_2_label[i] for i, _ in enumerate(pred_scores.tolist())], 'scores': pred_scores.tolist()})
            e.append(sim.data.item())
        t.append(sim.data.item())
    print('Eval_acc: {}\n'.format(eval_correct_num / len(eval_list)))
    print(np.mean(t), np.mean(c), np.mean(e))
# import json
# with open('analysis.json', encoding='utf-8', mode='w+') as f:

...
#         self.args = args
    ...
    def forward(self, input, reversed=False):
        # batch_size * seq_len
        if reversed:
            sent1, sent2 = input[:, 1, :], input[:, 0, :]
        else:
...
if torch.cuda.device_count() > 1:
    esim1 = nn.DataParallel(esim1, device_ids=[0, 1, 2, 3])
    esim2 = nn.DataParallel(esim2, device_ids=[0, 1, 2, 3])
    esim1.to(device)
    esim2.to(device)
...
eval_esim_s2(esim1, esim2, eval_list, myData_eval, EVAL_EPOCH, 0, 'log_eval_esim_with_label.log')

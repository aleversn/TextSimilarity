...
class RandomPreTrainDataset(PreTrainDataset):
    def __getitem__(self, idx):
        is_sim = 1 if idx % 2 != 0 else 0
        idx = idx / 2 if idx % 2 == 0 else (idx - 1) / 2
        idx = int(idx)
        ...
        if is_sim == 1:
            ...
            t = int(random.random() * (len(self.ori_list) - 1))
            tags = self.tokenizer(self.ori_list[t], add_special_tokens=True, max_length=self.padding_length, padding="max_length", truncation=True)["input_ids"]
        ...
        return torch.tensor([sentence, tags]), torch.tensor(is_sim)
    
    def __len__ (self):
        return len(self.ori_list) * 2

tokenizer = BertTokenizer.from_pretrained('./dataset/vocab')

myDataset = RandomPreTrainDataset(tokenizer, './dataset/pre_train_data', 100)
...
if torch.cuda.device_count() > 1:
    ...
    esim.to(device)
# model_dict = torch.load("./model/esim_pretrain/esim_pretrain_3.pth").module.state_dict()
...
for epoch in range(num_epochs):
    ...
    torch.save(esim, './model/esim_pretrain/esim_pretrain_{}.pth'.format(epoch + 1 + save_offset))
    WriteSDC('log_esim_pretrain.log', 'epoch: {} train_acc: {} loss: {}\n'.format(epoch + 1 + save_offset, train_acc / train_count, train_loss / train_count))
